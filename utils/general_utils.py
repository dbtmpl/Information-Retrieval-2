...
from onir.spec import clip_crop, pad_min_len


def apply_spec_batch_qqa(batch, input_spec, device=None):
    result = {}
    for k in batch:
        result[k] = []
        seq_type, len_mode, maxlen, minlen = None, None, None, None

        if k.startswith('query_') or k.startswith('question_') or k.startswith('answer_'):
